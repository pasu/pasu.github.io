...
# 引言
你是否曾经好奇，计算机如何理解形状和曲线？传统的微分几何主要研究平滑的曲线和表面，但看起来和计算机中的像素化或多边形化世界相去甚远。这就是**离散微分几何 DDG**的用武之地：它弥合了平滑数学和计算实用性之间的鸿沟。让我们一起探索什么是微分几何和DDG，它们的区别，DDG的宏大愿景，以及一个具体的例子来加深理解。

---

# 什么是微分几何？
**微分几何**是研究平滑形状（如曲线和表面）的数学分支。它研究曲线、表面以及更高维物体在局部和整体上的行为。通过使用微积分等工具，它帮助回答以下问题：
- **曲率**：曲线在某一点的弯曲程度是多少？

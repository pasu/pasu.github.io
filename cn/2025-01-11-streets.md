...
在 Streets.GL 中，我们可以通过 `Tile3DFromVectorProvider` 创建一个 tile：

```typescript
const providerParams: Tile3DProviderParams = { /* 按需初始化参数 */ };
...
在 `Tile3DFromVectorProvider` 中，关键组件包括：

1. **Tile3DFromVectorProvider：**  
   该类将矢量 tile 数据转换为可以在 3D 环境中渲染的 3D 要素集合。它处理矢量 tile 数据以生成各种 3D 几何图形，例如拉伸的建筑、投影线条以及其他特性。

2. **Tile3DExtrudedGeometry：**  
   这是一个接口，用于表示拉伸的 3D 几何体，通常用于 3D 建筑。它包含用于位置、UV 坐标、法线、纹理 ID、颜色和 ID 的多个缓冲区，以及几何体的边界框。

3. **RoofBuilder：**  
   这是一个定义用于创建拉伸 3D 建筑屋顶几何的构建器接口。它包括用于生成各种类型屋顶（如双坡屋顶、四坡屋顶和平顶屋顶）的方法和参数。

4. **Tile3DExtrudedGeometryBuilder：**  
   该类负责构建 `Tile3DExtrudedGeometry` 对象。它处理建筑物的墙壁、屋顶及其他拉伸特征的生成，使用多个专门的构建器处理特定类型的屋顶和其他细节。
